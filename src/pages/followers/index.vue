    ...
    <scroll-view class="scroll" scroll-y @scrolltolower="loadMore">
      <view class="user-item" v-for="u in users" :key="u.id">
        ...
        <view class="meta">
          ...
          <text class="bio">{{ u.bio || '这个人很神秘' }}</text>
        </view>
        <button class="btn" :class="{ primary: !u.followed }" @tap="toggle(u)">{{ u.followed ? '已关注' : '关注' }}</button>
      </view>
      <view class="loading" v-if="isLoadingMore">加载中...</view>
      <view class="no-more" v-if="noMore && users.length">没有更多了</view>
      <view class="empty" v-if="!users.length && !isLoading">暂无粉丝</view>
    </scroll-view>
  </view>
</template>
...
export default {
  ...
  methods: {
    async load() {
      ...
      this.isLoading = true
      try {
        const res = await request.get('/api/followers', { page: this.page, size: this.pageSize })
    ...
    async toggle(user) {
      try {
        if (user.followed) await request.post('/api/unfollow', { user_id: user.id })
        else await request.post('/api/follow', { user_id: user.id })
...
.btn { padding: 12rpx 24rpx; border-radius: 26rpx; background: #eee; color: #333; border: none; }
